...
class DescribeRecords:
    def it_emits_one_leaf_record_per_scalar(self):
        ...
        assert list(records(root, "heroes.vdata")) == [
            {"file": "heroes.vdata", "path": "hero_a.hp", "value": 100},
            {"file": "heroes.vdata", "path": "hero_a.name", "value": "A"},
        ]

    def it_indexes_sequences_in_the_path(self):
        root = {"hero_a": {"items": ["x", "y"]}}
        assert [r["path"] for r in records(root, "f")] == [
            "hero_a.items[0]",
            "hero_a.items[1]",
        ]
    ...
    def it_unwraps_flagged_values_in_leaf_values(self):
        ...
        assert list(records(root, "f")) == [
            {"file": "f", "path": "hero_a.model", "value": "models/a.vmdl"}
        ]
